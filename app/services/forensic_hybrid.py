...
def apply_verdict_score_band_consistency(output: dict[str, Any]) -> dict[str, Any]:
    ...
    AUTHENTIC 75–100, SUSPICIOUS 40–74, FAKE 0–39.
    ...
    if verdict not in ("AUTHENTIC", "SUSPICIOUS", "FAKE") or not isinstance(ts, int):
        return output
    ...
    if verdict == "AUTHENTIC" and ts < 75:
        flags.append(
            "Hybrid consistency: score below AUTHENTIC band (75+); verdict adjusted to SUSPICIOUS for review."
        )
        out["verdict"] = "SUSPICIOUS"
        out["flags"] = flags
        if summary_s:
            out["summary"] = summary_s + " (Adjusted by consistency check.)"
        return out
    ...
    if verdict == "SUSPICIOUS" and ts >= 75:
        flags.append(
            "Hybrid consistency: trust_score in AUTHENTIC range but verdict SUSPICIOUS — institutional confirmation recommended."
        )
        out["flags"] = flags
        return out

    if verdict == "SUSPICIOUS" and ts <= 39:
        flags.append(
            "Hybrid consistency: trust_score in FAKE range but verdict SUSPICIOUS — escalate review if policy requires."
        )
        out["flags"] = flags
        return out
